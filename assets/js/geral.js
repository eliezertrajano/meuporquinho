...
function carregarSelect_custom(name, consulta) {
	...
	$.ajax({
		url: "process/utils/listar_valores.php",
		dataType: "html",
		data: dados,
		async: false,
		success: function(data) {
...
function carregarValores(data) {
	...
	var funcao = '';
	var assinc = true;
	var urltipo = 'process/utils/listar_valores.php';
	...
	if(arguments[3]){
		urltipo= 'process/utils/listar.php';
	}
	$.ajax({
		url: urltipo,
		data: data,
...
function criarTabela() {
	...
	for (var k in itens) {
		...
		for (var j in itens[k]) {
			if(j!='id'){
					body = body + "<td> <a href='?q=" + arguments[2] + "&id=" + itens[k].id + "'>";
          body = body + itens[k][j];
			    body = body + "</a></td>";
			}	
...
function removerItens(dados, callback) {
	$.ajax({
		method: 'POST',
		data: dados,
		url: 'process/utils/remover_itens.php',
		success: callback
	});
}
